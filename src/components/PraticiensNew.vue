<template>
  ...
  >
    ...
    <div class="praticiens-grid">
      <!-- État de chargement -->
      <div v-if="loading" class="loading-state" style="grid-column: 1 / -1; text-align: center; padding: 2rem;">
        <p>🔄 Chargement des praticiens...</p>
      </div>

      <!-- État d'erreur -->
      <div v-else-if="error" class="error-state" style="grid-column: 1 / -1; text-align: center; padding: 2rem; background: #fee; border: 1px solid #fcc; border-radius: 8px;">
        <p>⚠️ Erreur de connexion à la base de données</p>
        <p class="error-message" style="color: red; font-size: 0.9rem;">{{ error }}</p>
